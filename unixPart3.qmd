# Redirection, Pipes, and Text Processing in Unix
...
The`grep` (**g**lobal **r**egular **e**xpression) command in Linux is used to search for specific patterns or strings within files and display the matching lines. It is a powerful tool for text processing and can be customized with various options to refine search results. The basic usage is: ***grep "searchword" filename***
...
a)   In the butterfly_ecology.csv file find all "Aglais" species and save the fields species, family, range.size in a new file.

b)   The shell keeps a record of the commands you have previously run. You can display this list using the `history` command. Using this information, determine how many times you have used the ls command in your shell history. Hint: You may need to combine `history` with other command-line tools such as `grep`.
